...
| [Testing / revising model details](modeling_details_revisions.md) | Ablations: P→I/M ITI gate, I/M −400→−100 ms zero penalty, `W_pp` slow-prior box, one vs two action thresholds; 3× pre-action M (08-31; 09-16 mpre3 × 150 / full scored); I/M prior at full 150 ms (09-07); `im150stim` / `stimonly` (09-08; open `g_i` floor worse; restore `0.1`); model I/M distance = `mean_c ‖Δ‖` (09-08f; im150 meancell rerun); 09-16b cross-window 80 vs 150 fair tables; 09-16c split 150 post-stim / 80 pre-move queued |
...
| [Stage B fit variants (catalog)](stage_b_fit_variants.md) | Index of every scored Stage B / joint tag: freeze, I/M window / stratum, S sidecar, best seed. Details stay in the topic journals; append a row when a new arm is scored. 09-16b fair 80 vs 150 tables; 09-16c split-window tags queued |
