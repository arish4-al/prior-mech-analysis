"""Act-prior perf/RT overlays for Stage B im150stim (Test 6 revised).

Same protocol as ``_tmp_im150_actprior_rt.py``: 10 sessions × 20
...
PREFIX = "weights_run_fj_stageB_hold_s89_im150stim_regular_mask12-13"
OUT = BASE / "stageB_hold_s89_im150stim_actprior_rt.json"
...
def main():
    ...
    rows = []
    print(
        f"{'model':<16} {'perf':>6} {'RTcomb':>7} {'RTspl':>6} "
        f"{'con':>6} {'inc':>6}",
        flush=True,
    )
    for seed in SEEDS:
        run = BASE / f"{PREFIX}_s{seed}"
        r = plot_one_json(f"im150stim_s{seed}", latest_final(run), run, behavior)
        rows.append(r)
        print(
