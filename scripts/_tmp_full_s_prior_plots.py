"""S / I/M / prior overlays + act-prior behavior for Stage B full+S.

Same shared stim as the eval (bps=20, seed 12345, from regular s101).
``include_stim=True`` overlays the unsplit-80 S sidecar. Act-prior RT
uses the same protocol as ``_tmp_perf_rt_model_vs_data.py``.

Plots go in each run dir (openalyx models/), not repo figs/.
"""
...
def main():
    ...
    for arm, prefix in arms.items():
        for seed in SEEDS:
            run = resolve_run(prefix, seed)
            if run is not None:
                jobs.append((arm, seed, run))
            else:
    ...
    for i, (arm, seed, run) in enumerate(jobs, 1):
        jp = newest_final(run)
        print(f"\n[traj {i}/{len(jobs)}] {arm} s{seed}  {run.name}", flush=True)
        s = plot_one(
            jp, stim_bundle, mean_data, prior_regions, run,
            avg_mean_R=avg_mean_R, include_stim=True,
        )
        plt.close("all")
        alias_svgs(run)
        s["arm"] = arm
