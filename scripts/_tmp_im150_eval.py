"""Shared-stim fair eval for Stage B im150 (Test 6).

bps=20, stim seed 12345, stim from baseline s101. Score im150 as fitted
(prior_window_ms=150) and with the flag cleared (legacy T=72). Baseline
regular is scored at T=72 (production) and at 150 ms (same new objective).
"""
...
OUT = BASE / "stageB_hold_s89_im150_eval.json"
...
def main():
    ...
    rows = []
    print(
        f"{'arm':7} {'seed':>4} {'win':>4} {'rec':>7} {'traj':>7} {'prior':>7} "
        f"{'fair':>7} {'gi':>6} {'di':>6} {'θc':>5} {'θd':>5}",
        flush=True,
    )
    for arm, prefix in (
        ("im150", "weights_run_fj_stageB_hold_s89_im150_regular_mask12-13"),
        ...
        for seed in SEEDS:
            ...
            windows = (150.0, None) if arm == "im150" else (None, 150.0)
            for win in windows:
                mp_sc = dict(mp)
                ...
                print(
                    f"{arm:7} {seed:4d} {wlab:>4} {ev['recorded']:7.3f} "
                    f"{ev['eval_traj']:7.3f} {ev['eval_prior']:7.3f} "
                    f"{ev['eval_fair']:7.3f} "
                    f"{ev['g_i']:6.1f} {ev['d_i']:6.1f} "
