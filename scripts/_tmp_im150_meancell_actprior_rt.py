"""Act-prior perf/RT overlays for Stage B im150_meancell.

Same protocol as ``_tmp_im150_actprior_rt.py``: 10 sessions × 20 blocks,
stim seed 12345, data = action-kernel α=0.2, model = binarized
...
PREFIX = "weights_run_fj_stageB_hold_s89_im150_meancell_regular_mask12-13"
OUT = BASE / "stageB_hold_s89_im150_meancell_actprior_rt.json"
...
def main():
    ...
    for seed in SEEDS:
        ...
        r = plot_one_json(f"meancell_s{seed}", latest_final(run), run, behavior)
        rows.append(r)
        print(
