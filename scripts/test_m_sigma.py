#!/usr/bin/env python
"""Smoke for M-ODE diffusion (σ_M=0 identity, σ_M>0 changes RT, backends match)."""
...
from simulate_recovery import attach_m_sigma, m_sigma_dir_tag
...
def test_cache_tag():
    mp = {"g_s": 0.0}
    attach_m_sigma(mp, 0.0)
    assert "m_sigma" not in mp
    assert m_sigma_dir_tag(0.0) == ""
    attach_m_sigma(mp, 0.1)
    assert mp["m_sigma"] == 0.1
    assert m_sigma_dir_tag(0.1) == "_msig0.1"


def test_sigma0_numpy_numba_match():
...
def test_sigma_positive_changes_rt_and_matches_across_backends():
    mp, stim, ts, tside, bside, bps, sbo = _tiny_stim(seed=7)
    extra = {"m_sigma": 0.1, "m_sigma_rng_seed": 12345}
    r_det = _run("numpy", mp, stim, ts, tside, bside, bps, sbo)
    r_np = _run("numpy", mp, stim, ts, tside, bside, bps, sbo, **extra)
    r_nb = _run("numba", mp, stim, ts, tside, bside, bps, sbo, **extra)
    rt_det = np.asarray(r_det["reaction_time"])
    rt_n = np.asarray(r_np["reaction_time"])
    assert rt_det.shape == rt_n.shape
    ch_det = np.asarray(r_det["choices"])
    ch_n = np.asarray(r_np["choices"])
    m_len_changed = np.asarray(r_det["M"]).shape != np.asarray(r_np["M"]).shape
    assert m_len_changed or (not np.array_equal(rt_det, rt_n)) or (
        not np.array_equal(ch_det, ch_n)
    ), "σ_M=0.1 should change RT, choice, or trial length"
    for key in ("choices", "correct_action_taken", "reaction_time"):
        an = np.asarray(r_np[key])
        bn = np.asarray(r_nb[key])
        assert an.shape == bn.shape and np.array_equal(an, bn), key
    assert np.allclose(
        np.asarray(r_np["M"], dtype=float),
        np.asarray(r_nb["M"], dtype=float),
        rtol=1e-12,
        atol=1e-12,
        equal_nan=True,
    )
    ...
    i_det = np.asarray(r_det["I"], dtype=float)
    i_n = np.asarray(r_np["I"], dtype=float)
    # I is not directly noised; it can still differ via P←I←... feedback if M
    # changes choice/RT and thus trial length. Compare only overlapping prefix
    # is too messy; just require I is finite.
    assert np.isfinite(i_n).all() and np.isfinite(i_det).all()


if __name__ == "__main__":
    test_cache_tag()
    test_sigma0_numpy_numba_match()
    test_sigma_positive_changes_rt_and_matches_across_backends()
    print("m_sigma smoke OK")
