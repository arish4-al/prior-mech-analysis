"""Act-prior perf/RT for gm0 / mleak. Same 10×20, stim seed 12345."""
...
OUT = BASE / "stageB_hold_s89_gm0_mleak_eval" / "actprior_rt.json"
...
def main():
    ...
    for arm, prefix in ARMS:
        for seed in SEEDS:
            ...
            r = plot_one_json(f"{arm}_s{seed}", latest_final(run), run, behavior)
            r["arm"] = arm
            r["seed"] = seed
            rows.append(r)
            print(
                f"{arm:6} {seed:4d} {r['perf_r2']:.3f}  {r['rt_r2']:7.3f}  "
                f"{r['rt_split_r2']:.3f}  "
